...
export class LanguageService {
  ...
  async getLanguageList(page: number, limit: number) {
    try {
      const skip = (page - 1) * limit;

      const [getList, totalCount] = await Promise.all([
        this.languageModel.find().skip(skip).limit(limit),
        this.languageModel.countDocuments(),
      ]);
      return {
        statusCode: HttpStatus.OK,
  ...
  async getLanguageById(req: languageDto) {
    try {
      const findLanguage = await this.languageModel.findOne({
  ...
  async updateLanguage(req: languageDto) {
    try {
      ...
      if (findLanguage) {
        const modifyLanguage = await this.languageModel.updateOne(
          { languageId: req.languageId },
          {
            $set: {
  ...
  async deleteLanguage(req: languageDto) {
    try {
      const removeLanguage = await this.languageModel.deleteOne({
        languageId: req.languageId,
      });
      if (removeLanguage) {
        return {
